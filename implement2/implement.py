...
if HEURISTIC_1 or HEURISTIC_2 or HEURISTIC_3:
...
if HEURISTIC_3:
    K = 1000
    mutual = [0] * len(words)
    total_word_presences = [sum([word_presences[i][j] for i in range(len(class_names))]) for j in range(words)]
    

    # Calculate P(X = x), P(Y = y), and P(X = x, Y = x) and use mutual
    # information formula
